...
public class PlayerController : MonoBehaviour, IInputListener, ILightEmitter
{
	...
	private bool _yButtonCurrentlyPressed;
	private bool _yButtonPreviouslyPressed;
	...
	private List<Chest> _overlappingChests = new List<Chest> ();
	...
	void Start ()
	{
		Global.GameController.GetComponent<InputHandler> ().AddInputListener (this, InputHandler.JOYSTICK_NAMES [_controllerIndex]);
	...
	void Update ()
	{
		...
		//handle B: dash
		if (!_bButtonPreviouslyPressed && _bButtonCurrentlyPressed) {
			if (!_isDead && !Global.GameController.IsPaused) {
		...
		//handle X: interact with machine
		else if (!_xButtonPreviouslyPressed && _xButtonCurrentlyPressed) {
		...
		//handle Y: interact with chest
		else if (!_yButtonPreviouslyPressed && _yButtonCurrentlyPressed) {
			if (_overlappingChests.Count > 0) {
				Chest nearestChest = null;
				float shortestDistance = float.MaxValue;

				foreach (Chest chest in _overlappingChests) {
					Vector3 vector = chest.transform.position - this.transform.position;
					float distance = vector.magnitude;
		...
		if (!Global.GameController.IsPaused && Global.GameController.WindForce.magnitude > 0) {
			totalAcceleration += Global.GameController.WindForce;
		}
		...
		/*if (!_isDead && !Global.GameController.IsPaused && !_isDashing && !_isBumped) {
	...
	void OnTriggerEnter2D (Collider2D other)
	{
		...
		case "Machine":
			...
			if (machine.IsActive) {
				Chest chest = machine as Chest;

				if (chest != null && !_overlappingChests.Contains (chest)) {
					_overlappingChests.Add (chest);
				}
				if (!_overlappingMachines.Contains (machine)) {
	...
	void OnTriggerExit2D (Collider2D other)
	{
		...
		case "Machine":
			...
			if (chest != null) {
				_overlappingChests.Remove (chest);
			} else {
	...
	void IInputListener.OnHandleLeftStick (int joystickIndex, Vector2 stickState)
	{
		...
		if (joystickIndex == _controllerIndex && !_isDead && !Global.GameController.IsPaused) {
	...
	void IInputListener.OnHandleYButton (int joystickIndex, bool pressed)
	{
		...
		_yButtonPreviouslyPressed = _yButtonCurrentlyPressed;
		_yButtonCurrentlyPressed = pressed;
	}
